...
*                          erzet DESCENDING.
  ...
  METHOD save_data.
    ...
    LOOP AT im_data INTO DATA(ls_item).
      ...
      ls_item-item = COND #( WHEN line_exists( im_data_db[ inc = ls_item-inc ] )
                             THEN im_data_db[ inc = ls_item-inc ]-item
                             ELSE 1 ).
  ...
  METHOD get_data_bd.
    ...
    " Recuperar o registro mais recente de cada incidente
    SORT lt_data BY inc  ASCENDING
                    item DESCENDING.

    DELETE ADJACENT DUPLICATES FROM lt_data COMPARING inc.
